...
public class Solution {
    ...
    public ListNode reverseKGroup(ListNode head, int k) {
        final ListNode res = doReverse(head, k);
        return res == flag ? head : res;
    }
    ...
    public ListNode doReverse(ListNode head, int k) {
        ...
        // 递归调用,返回的是本组节点的头节点
        ListNode newHead = doReverse(head.next, k);
        // 将后一个节点指向前一个节点
        head.next.next = head;
        ...
        // 当前节点的后继节点如何处理
        // 1.
        if (count == 0 && nextHead != null && nextHead.next != null) {
            final ListNode tNode = nextHead;
            nextHead = null;
            head.next = doReverse(tNode, k);
        } else {
            head.next = null;
        }
